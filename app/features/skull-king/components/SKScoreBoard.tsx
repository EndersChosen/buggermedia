...
export function SKScoreBoard({ game, onEditRound }: SKScoreBoardProps) {
  ...
  const getSortedPlayersWithRanks = (): Array<{ player: Player; rank: number; rankLabel: string }> => {
    ...
    const playersWithRanks: Array<{ player: Player; rank: number; rankLabel: string }> = [];

    playersWithScores.forEach((item, index) => {
      let rank = index + 1;

      // Check if tied with previous player
      if (index > 0 && item.score === playersWithScores[index - 1].score) {
        rank = playersWithRanks[index - 1]?.rank || rank;
      }
